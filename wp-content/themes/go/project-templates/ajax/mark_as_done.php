...
if($_POST && $_POST['quote_id'] && $_POST['quote_milestone']) {
        ...
        foreach($payments as $payment) { $i++;
                if($i == $quote_milestone) {
						$new_object_id = go_create_invoice($quote_id, $client_id, $quote_milestone);
                        /*
        ...
        */
        ...
        $title = 'You have a new invoice: ' . $milestone_title;
        $text = '<p>Here is your new invoice: ' . $milestone_title . '.</p><p>Project details: ' . get_bloginfo('url') . '/?p=' . $quote_id . '</p><p>New invoice was generated, you can see details: ' . get_bloginfo('url') . '/?p=' . $new_object_id . '</p>';
        go_email($subject,$title,$text,$client_id);*/
		go_send_invoice_email($new_object_id, 'create', $who, $milestone_title);
