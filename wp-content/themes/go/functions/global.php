...
function go_userdata($id) {
    ...
    $object->address = get_field('address','user_' . $id);
		$object->type = get_field('user_type','user_' . $id);
                ...
                $object->company = get_field('company_name','user_' . $id);
...
function go_projects_statistic($id) {
	...
	if($user_data != false) {
      ...
      $paid = get_field('paid',$project->ID);
		...
		$object->cancelled_total = $cancelled_total;
...
function go_project_status($id) {
	...
	if($status == 'quote') {
		$object->status = 'quote';
		$object->status_string = 'Pre Quote';
	...
	elseif($status == 'live') {
		$object->status = 'live';
		$object->status_string = 'Accepted Quote';
		$object->status_class = 'info';
	}
	elseif($status == 'completed') {
		$object->status = 'completed';
		$object->status_string = 'Completed';
		$object->status_class = 'success';
	}
	elseif($status == 'cancelled') {
		$object->status = 'cancelled';
		$object->status_string = 'Closed';
